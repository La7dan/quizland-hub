...
import { UploadCloud, Search, FileText, CheckCircle, XCircle, Clock, Download } from 'lucide-react';
...
const EvaluationManagement: React.FC<EvaluationManagementProps> = ({ onRefresh }) => {
  ...
  const getSelectedMemberCode = (): string => {
    if (!selectedMemberId || !membersData) return '';
    
    const selectedMember = membersData.find((member: any) => 
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const memberCode = getSelectedMemberCode();
      if (!memberCode) {
            ...
            <TabsContent value="upload" className="space-y-4">
              ...
              <div className="flex justify-end">
                <Button 
                  ...
                  className="gap-2"
                  disabled={isUploading || !selectedMemberId || !evaluationDate || !pdfFile}
                >
